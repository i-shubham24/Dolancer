    ...
    <div className="mx-auto w-full max-w-3xl px-4 py-16 lg:px-6 lg:py-20">
      <nav className="mb-8 flex gap-2" aria-label="Legal documents">
        {(["terms", "privacy"] as const).map((option) => (
          <Link key={option} to={`/legal/${option}`} className={cn("rounded-full border border-line-card px-4 py-2 text-sm font-extrabold transition-all duration-[120ms]", option === resolved ? "bg-lime shadow-soft-sm" : "bg-surface hover:shadow-soft-sm")}>
