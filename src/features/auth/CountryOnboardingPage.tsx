...
export function CountryOnboardingPage() {
  ...
  return (
    <motion.div initial={rise} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, ease: [0.16, 1, 0.3, 1] }} className="relative w-full max-w-2xl py-4 sm:py-8">
      <div className="pointer-events-none absolute -left-16 top-0 h-32 w-32 rounded-full bg-highlight-light blur-3xl" aria-hidden="true" />
      <div className="pointer-events-none absolute -right-12 bottom-4 h-40 w-40 rounded-full bg-secondary-light blur-3xl" aria-hidden="true" />
      ...
      <div className="relative mb-7">
        <p className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-highlight"><Sparkles className="h-4 w-4" aria-hidden="true" /> One quick step</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full rounded-xl border border-line-card bg-surface px-4 py-3 text-sm font-medium shadow-soft-sm outline-none transition-all focus:border-highlight focus:ring-4 focus:ring-highlight-light"
            >
              {COUNTRIES.map((entry) => (
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        <StitchColorCard tone="lilac" className="p-4">
          <Globe2 className="mb-3 h-5 w-5 text-highlight" />
